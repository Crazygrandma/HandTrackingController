...
def track_hands(cap, draw=True):
    success, img = cap.read()  # get BGR img from webcam
    img = cv2.flip(img, 1)  # flipping image for easier controls
    detector.find_hands(img, draw=UISettings.DrawHands)  # returns RGB img, displays hand landmarks, when draw=True
    # returns list of landmarks with (id, x position, y position)
    rightHand = detector.find_position(img, label='Right', draw=UISettings.DrawHandPoints)
    leftHand = detector.find_position(img, label='Left', draw=UISettings.DrawHandPoints)

    gestures, wrist1, wrist2 = get_gestures(rightHand, leftHand)

    if draw:
        draw_on_screen(img, wrist1, wrist2)
...
def get_gestures(rightHand, leftHand):
    if len(rightHand) and len(leftHand):
        ...
        rightJoystick = (
            (wrist1[1] - p1[0]) / UISettings.JoystickRadius, (wrist1[2] - p1[1]) / UISettings.JoystickRadius)
        leftJoystick = (
            (wrist2[1] - p2[0]) / UISettings.JoystickRadius, (wrist2[2] - p2[1]) / UISettings.JoystickRadius)
...
def draw_on_screen(img, rightHand, leftHand):
    p1 = UISettings.p1
    p2 = UISettings.p2
